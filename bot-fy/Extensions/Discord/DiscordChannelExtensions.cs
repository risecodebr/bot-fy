...
public static class NotificationsDiscordChannelExtensions
{
    ...
    public static async Task SendPaginatedMusicsAsync(this DiscordChannel channel, DiscordUser user, IEnumerable<LavalinkTrack> videos)
    {
        ...
        {
            List<LavalinkTrack> page = videos.Skip(i * 10).Take(10).ToList();
            ...
            {
                embed.Description += $"{(i * 10) + j} - {Formatter.MaskedUrl(page[j].Title, new Uri(page[j].Uri.ToString()))}\n";
            }
            embed.WithFooter($"Página {i + 1} de {pages_count} - {videos.Sum(p => p.Length).ToStringTime()}");
